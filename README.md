...
#### 图片编辑模型（支持参考图片）
- **FLUX.1 Kontext [dev]** - 前沿图片编辑模型，理解图片上下文，编辑更精准（默认推荐）
- **FLUX.1 Kontext Max** - 更强大的模型，处理复杂任务
...
- **分辨率模式** - 替代传统图片比例设置，自动选择（默认推荐）/匹配输入图片/手动指定比例

#### 高级设置
- **推理步数** - 控制生成质量，默认28步，更多步数质量更高但速度更慢
- **生成加速** - 标准速度（默认）/常规加速/高速加速，加速可能略微影响质量
- **安全检查器** - 检查生成内容是否包含不当内容，默认关闭
...
- FLUX.1 Kontext [dev] 使用新的参数体系，提供更精细的控制
