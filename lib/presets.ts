...
export const EDIT_PRESET_SYSTEM: PresetSystem = {
 ...
 * 构建完整的预设提示词
 */
export function buildPresetPrompt(preset: EditPreset, subject?: string): string {
  let brief = preset.brief
  
  // 处理 {{SUBJECT}} 占位符
  if (subject && brief.includes('{{SUBJECT}}')) {
    brief = brief.replace('{{SUBJECT}}', subject)
  } else if (brief.includes('{{SUBJECT}}')) {
    // 如果没有提供 subject，移除占位符
    brief = brief.replace('{{SUBJECT}} ', '')
  }
  
  return `${EDIT_PRESET_SYSTEM.prefix}\n\n${brief}\n\n${EDIT_PRESET_SYSTEM.suffix}`
}
 ...
 * 根据名称获取预设
 */
export function getPresetByName(name: string): EditPreset | undefined {
  return EDIT_PRESET_SYSTEM.presets.find(preset => preset.name === name)
}
